"""Train a binary SkipPoolCNN for identifier presence detection.
...
as "identifier present". The quadrant column is intentionally ignored.
"""
...
except ModuleNotFoundError:  # Allows --scan_only without plotting dependencies.
    plt = None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and export a binary TensorFlow/TFLite SkipPoolCNN for identifier presence."
    )
    parser.add_argument("--base_dir", type=Path, default=Path("."), help="Project directory.")
    parser.add_argument(
        "--output_dir",
        type=Path,
        default=Path("outputs") / "skippool_presence_binary_improved",
    ...
    parser.add_argument(
        "--model_variant",
        choices=("multiscale", "compact"),
        default="multiscale",
        help="multiscale is more accurate; compact preserves the original 6713-parameter model.",
    )
    parser.add_argument(
...
def require_runtime() -> None:
    if tf is None or np is None or plt is None:
        raise SystemExit(
            "Training dependencies are not installed in this Python environment.\n"
...
def build_skippool_presence_model(
    img_size: int,
    learning_rate: float,
    model_variant: str = "multiscale",
) -> "tf.keras.Model":
    """Build SkipPoolCNN with one sigmoid output: absent=0, present=1."""
    ...
    # The skip branches preserve bright/dark low-resolution evidence that can
    # disappear in the learned convolutional path.
    ...
    if model_variant == "compact":
        conv_spec = ((4, 2), (8, 2), (12, 2), (12, 1), (16, 1))
        dense_units = 0
        use_batch_norm = False
        skip_tensors = [skip_max]
    elif model_variant == "multiscale":
        conv_spec = ((6, 2), (12, 2), (18, 2), (24, 1))
        dense_units = 24
        use_batch_norm = True
        skip_avg = tf.keras.layers.AveragePooling2D(
            ...
            strides=8,
            padding="valid",
            name="skip_average_pool",
        )(scaled)
        skip_tensors = [skip_max, skip_avg]
    else:
        raise ValueError(f"Unsupported model_variant: {model_variant}")

    x = scaled
    for index, (filters, stride) in enumerate(conv_spec):
        ...
        if model_variant == "compact" and index < 3:
            x = tf.keras.layers.MaxPooling2D(
                3, strides=1, padding="same", name=f"pool_{index}"
            )(x)

    x = tf.keras.layers.Concatenate(axis=-1, name="concat_skip")([x, *skip_tensors])
    ...
    model = tf.keras.Model(inputs=inputs, outputs=outputs, name="skippool_identifier_presence_binary")
    model.compile(
...
def plot_training_history(history: "tf.keras.callbacks.History", output_path: Path) -> None:
    """Plot loss, accuracy and AUC for training and validation."""
    ...
    for axis, (key, title) in zip(axes, metrics):
        if key in history.history:
            axis.plot(epochs, history.history[key], label="Entrenamiento", color="#1769aa")
        validation_key = f"val_{key}"
        if validation_key in history.history:
            axis.plot(
                epochs,
                history.history[validation_key],
                label="Validacion",
                color="#d97706",
                linestyle="--",
            )
        axis.set(title=title, xlabel="Epoca", ylabel=title)
        if key != "loss":
            axis.set_ylim(0, 1)
        axis.grid(alpha=0.25)
        axis.legend()
    fig.suptitle("Entrenamiento SkipPoolCNN binario")
    fig.tight_layout()
...
def plot_threshold_analysis(
    ...
    """Show how binary metrics change with the decision threshold."""
    thresholds = np.linspace(0.05, 0.95, 91)
    rows = [metrics_from_probabilities(labels, probabilities, float(value)) for value in thresholds]
    fig, axis = plt.subplots(figsize=(8.5, 5))
    for key, label, color in (
        ("precision", "Precision", "#1769aa"),
        ("recall", "Recall", "#d97706"),
        ("specificity", "Especificidad", "#16836b"),
        ("f1", "F1", "#8b5cf6"),
    ):
        axis.plot(thresholds, [float(row[key]) for row in rows], label=label, color=color)
    axis.axvline(selected_threshold, color="#202020", linestyle="--", label=f"Elegido: {selected_threshold:.2f}")
    axis.set(
        title="Metricas de validacion segun el umbral",
        xlabel="Umbral de presencia",
        ylabel="Metrica",
        xlim=(0.05, 0.95),
        ylim=(0, 1),
    )
...
def plot_binary_confusion_matrix(
    metrics: dict[str, float | int], output_path: Path, title: str
) -> None:
    """Plot the binary confusion matrix as counts and row percentages."""
    matrix = np.array(
...
def predict_tflite_probabilities(
    ...
    """Run the exported TFLite model and return dequantized probabilities."""
    interpreter = tf.lite.Interpreter(model_content=model_bytes)
    interpreter.allocate_tensors()
    input_detail = interpreter.get_input_details()[0]
    output_detail = interpreter.get_output_details()[0]
    ...
    dataset = make_dataset(samples, img_size, batch_size=1, training=False, seed=0)
    for image, _label in dataset:
        input_value = image.numpy()
        if np.issubdtype(input_detail["dtype"], np.integer):
            quantized = np.round(input_value / input_scale + input_zero_point)
            limits = np.iinfo(input_detail["dtype"])
            input_value = np.clip(quantized, limits.min, limits.max).astype(input_detail["dtype"])
        else:
            input_value = input_value.astype(input_detail["dtype"])
        interpreter.set_tensor(input_detail["index"], input_value)
        interpreter.invoke()
        output_value = interpreter.get_tensor(output_detail["index"]).reshape(-1)[0]
        if np.issubdtype(output_detail["dtype"], np.integer):
            output_value = (float(output_value) - output_zero_point) * output_scale
        probabilities.append(float(output_value))
    return np.asarray(probabilities, dtype=np.float32)


def write_c_array(model_bytes: bytes, output_dir: Path, var_name: str) -> tuple[Path, Path]:
    header_path = output_dir / "skippool_presence_binary_model_data.h"
    source_path = output_dir / "skippool_presence_binary_model_data.cc"
...
def path_for_report(path: Path, base_dir: Path) -> str:
    """Prefer a project-relative path, while supporting absolute output directories."""
    try:
...
def write_report(
    ...
    lines.extend(
        [
            ...
            "## Model",
            "",
            f"- Architecture: `SkipPoolCNN binary ({args.model_variant})`",
            f"- Input: `{args.img_size}x{args.img_size}x1` grayscale, raw pixel range 0..255.",
            f"- Total parameters: {model.count_params()}",
            f"- Estimated FP32 parameter memory: `{model.count_params() * 4 / (1024 ** 2):.4f} MB`.",
    ...
    if model_sizes_mb:
        ...
        for name, size_mb in model_sizes_mb.items():
            lines.append(f"| {name} | {size_mb:.4f} |")

    lines.extend(["", "## Artifacts", ""])
    for name, artifact_path in artifacts.items():
        lines.append(f"- {name}: `{artifact_path}`")

    if any(name.startswith("plot_") for name in artifacts):
        lines.extend(
            [
                "",
                "## Evaluation Plots",
...
def main() -> int:
    ...
    model = build_skippool_presence_model(args.img_size, args.learning_rate, args.model_variant)
    class_weight = compute_class_weight(train, args.positive_weight_multiplier)
    checkpoint_path = output_dir / "best_skippool_presence_binary.keras"
    ...
    threshold_plot_path = plots_dir / "threshold_analysis.png"
    confusion_plot_path = plots_dir / "confusion_matrix_test.png"
    plot_training_history(history, training_plot_path)
    val_probabilities = predict_probabilities(model, val, args.img_size, args.batch_size)
    val_labels = np.array([sample.label for sample in val], dtype=np.int32)
    plot_threshold_analysis(val_labels, val_probabilities, threshold, threshold_plot_path)
    plot_binary_confusion_matrix(metrics["test"], confusion_plot_path, "Matriz de confusion - Prueba")

    artifacts = {
        "keras": path_for_report(checkpoint_path, base_dir),
        "history": path_for_report(output_dir / "history.csv", base_dir),
        "dataset_summary": path_for_report(output_dir / "dataset_summary.csv", base_dir),
        "plot_training_curves": path_for_report(training_plot_path, base_dir),
        "plot_threshold_analysis": path_for_report(threshold_plot_path, base_dir),
        "plot_confusion_matrix_test": path_for_report(confusion_plot_path, base_dir),
    }
    tflite_info: dict[str, dict[str, object]] = {}

    float_tflite = convert_float_tflite(model, args.img_size)
    float_path = output_dir / "skippool_presence_binary_float32.tflite"
    ...
    if not args.no_int8:
        try:
            ...
            int8_path = output_dir / "skippool_presence_binary_int8.tflite"
            ...
            int8_probabilities = predict_tflite_probabilities(int8_tflite, test, args.img_size)
            test_labels = np.array([sample.label for sample in test], dtype=np.int32)
            metrics["test_int8"] = metrics_from_probabilities(
                test_labels, int8_probabilities, threshold
            )

            if not args.no_c_array:
                cc_path, h_path = write_c_array(int8_tflite, output_dir, "g_skippool_presence_binary_model")
    ...
    model_sizes_mb = {
        "keras": checkpoint_path.stat().st_size / (1024 ** 2),
        "tflite_float32": len(float_tflite) / (1024 ** 2),
    }
    if "tflite_int8" in artifacts:
        model_sizes_mb["tflite_int8"] = int8_path.stat().st_size / (1024 ** 2)
    ...
    artifacts["report"] = path_for_report(output_dir / "report.md", base_dir)
